...
- PK (REQUIRED), a public key in the format output by SkToPk.
...
- SK (REQUIRED), a secret key of the form outputted by the KeyGen operation.
...
- PK (REQUIRED), a public key of the form outputted by the SkToPk operation.
- signature (REQUIRED), octet string.
- header (OPTIONAL), an octet string containing context and application specific
...
```
proof = ProofGen(PK, signature, ph, header, (msg_1,..., msg_L), RevealedIndexes)

Inputs:

- PK (REQUIRED), octet string in output form from SkToPk.
- signature (REQUIRED), octet string in output form from Sign.
- ph (REQUIRED), octet string.
- header (OPTIONAL), an octet string containing context and application specific
          information. If not supplied, it defaults to an empty string.
- msg_1,..., msg_L (OPTIONAL), an optional vector of octet strings. Messages in input to Sign.
